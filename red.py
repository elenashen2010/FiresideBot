...
def loadHelp():
	...
	{0}iam [role] - Adds yourself to a self-assignable role
	{0}iamnot [role] - Removes yourself from a self-assignable role
	{0}lsar - Lists all the avialable self-assignable roles
	...
	**Admin commands:**
	...
	{0}asar [role] - Adds a role to the list of self-assignable roles
	{0}rsar [role] - Removes a role from the list of self-assignable roles
	""".format(settings["PREFIX"])
...
async def on_message(message):
	...
	if not message.channel.is_private and message.author.id != client.user.id:
		...
		if  message.channel.id not in shush_list:
			...
			elif message.content.startswith(p + "iamnot"):
				await selfUnassignRole(message)
			elif message.content.startswith(p + "iam"):
				await selfAssignRole(message)
			elif message.content == p + "lsar":
				await listSARoles(message)
			elif message.content == p + "pc":
				await togglePcRole(message)
			elif message.content == p + "xbox":
				await toggleXboxRole(message)
			elif message.content == p + "ps4":
				await togglePsRole(message)
			################################################
			...
			######## Self-assignable Roles Admin Commands ########
			elif message.content.startswith(p + "asar"):
				await addSARole(message)
			elif message.content.startswith(p + "rsar"):
				await removeSARole(message)
			###################################
...
async def selfAssignRole(message):
	global saRoles
	member = message.author
	p = settings["PREFIX"]
	msg = message.content.split(" ")
	if message.channel.server.id not in saRoles:
		return
	if len(msg) != 2:
		await client.send_message(message.channel, "`" + settings["PREFIX"] + "iam [role]`")
		return
	if msg[1] not in saRoles[message.server.id]:
		await client.send_message(message.channel, "`" + msg[1] + " is not a self-assignable role.`")
		return	
	role = discord.utils.get(message.server.roles, name=msg[1])
	if role == None:
		await client.send_message(message.channel, "`" + msg[1] + " is not an actual role.`")
		return
	if role in member.roles:
		await client.send_message(message.channel, "`You already have the " + role.name + " role.`")
	await client.add_roles(member, role)
	await client.send_message(message.channel, "`You now have the " + role.name + " role.`")

async def selfUnassignRole(message):
	global saRoles
	member = message.author
	p = settings["PREFIX"]
	msg = message.content.split(" ")
	...
	if role not in member.roles:
		await client.send_message(message.channel, "`You are already don't have " + role.name + " role.`")
	await client.remove_roles(member, role)
	await client.send_message(message.channel, "`You no longer have the " + role.name + " role.`")

async def listSARoles(message):
	if message.channel.server.id not in saRoles:
		return
	saRolesList = saRoles[message.channel.server.id]
	await client.send_message(message.author, "**Self-Assignable Roles List:**\n	"+"\n	".join(saRolesList))
	await client.send_message(message.channel, "{} `Check your DMs for the self-assignable roles list.`".format(message.author.mention))

async def addSARole(message):
	global saRoles
	if isMemberAdmin(message):
		msg = message.content.split()
		if len(msg) >= 2:
			del msg[0]
			if not message.server.id in saRoles:
				saRoles[message.server.id] = []
			for word in msg:
					if word.find("/") != -1:
						word = word.replace("/", " ")
					saRoles[message.server.id].append(word)
			await client.send_message(message.channel, "`Updated self-assignable roles database.`")
			dataIO.fileIO("json/sa_roles.json", "save", saRoles)
			logger.info("Saved self-assignable roles.")
		else:
			await client.send_message(message.channel, "`" + settings["PREFIX"] + "asar [role1] [role2] [role/with/spaces] (...)`")
	else:
		await client.send_message(message.channel, "`I don't take orders from you.`")

async def removeSARole(message):
	global saRoles
	if isMemberAdmin(message):
		msg = message.content.split()
		if len(msg) >= 2:
			del msg[0]
			if message.server.id in saRoles:
				for w in msg:
					try:
						if w.find("/") != -1:
							w = w.replace("/", " ")
						saRoles[message.server.id].remove(w)
					except:
						pass
				await client.send_message(message.channel, "`Updated self-assignable roles database.`")
				dataIO.fileIO("json/sa_roles.json", "save", saRoles)
			...
			await client.send_message(message.channel, "`" + settings["PREFIX"] + "rsar [role1] [role2] [role/with/spaces] (...)`")
	else:
		await client.send_message(message.channel, "`I don't take orders from you.`")

async def togglePcRole(message):
	member = message.author
	commandMessage = message
	if discord.utils.get(member.roles, name="PC"):
		commandMessage.content = "!iamnot PC"
		await selfUnassignRole(commandMessage)
	else:
		commandMessage.content = "!iam PC"
		await selfAssignRole(commandMessage)

async def toggleXboxRole(message):
	member = message.author
	commandMessage = message
	if discord.utils.get(member.roles, name="XBOX"):
		commandMessage.content = "!iamnot XBOX"
		await selfUnassignRole(commandMessage)
	else:
		commandMessage.content = "!iam XBOX"
		await selfAssignRole(commandMessage)

async def togglePsRole(message):
	member = message.author
	commandMessage = message
	if discord.utils.get(member.roles, name="PS4"):
		commandMessage.content = "!iamnot PS4"
		await selfUnassignRole(commandMessage)
	else:
		commandMessage.content = "!iam PS4"
		await selfAssignRole(commandMessage)
...
def loadDataFromFiles(loadsettings=False):
	global proverbs, commands, trivia_questions, badwords, badwords_regex, shush_list, twitchStreams, blacklisted_users, apis, saRoles

	saRoles = dataIO.fileIO("json/sa_roles.json", "load")
	logger.info("Loaded " + str(len(saRoles)) + " self-assignable roles.")
	logger.info(saRoles)
